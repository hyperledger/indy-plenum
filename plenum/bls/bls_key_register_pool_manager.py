...
from plenum.common.constants import BLS_KEY, BLS_KEY_PROOF, ALIAS, POOL_LEDGER_ID, NODE
...
class BlsKeyRegisterPoolManager(BlsKeyRegister):
    ...
    def get_pool_root_hash_committed(self):
        return self._node.states[POOL_LEDGER_ID].committedHeadHash
    ...
    def _load_keys_for_root(self, pool_state_root_hash):
        ...
        for data in self._node.write_manager.get_main_req_handler(NODE). \
                get_all_node_data_for_root_hash(pool_state_root_hash):
            node_name = data[ALIAS]
            if BLS_KEY in data:
                if not self._node.config.VALIDATE_BLS_SIGNATURE_WITHOUT_KEY_PROOF and \
