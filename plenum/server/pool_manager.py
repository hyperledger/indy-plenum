...
class TxnPoolManager(PoolManager, TxnStackManager):
    ...
    def nodeServicesChanged(self, txn_data):
        ...
        if oldServices == newServices:
            logger.warning("Node {} not changing {} since it is same as existing".format(nodeNym, SERVICES))
            return
        else:
            if self.name != nodeName:
                ...
                if VALIDATOR in oldServices.difference(newServices):
                    ...
                    try:
                        ...
                        logger.warning('{} did not find remote {} to remove'.format(self, nodeName))
