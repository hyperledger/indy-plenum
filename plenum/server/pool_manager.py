...
class HasPoolManager:
    def __init__(self, nodeRegistry=None, ha=None, cliname=None, cliha=None):
        if not nodeRegistry:
            ...
            for types in (NEW_NODE, NEW_STEWARD, NEW_CLIENT):
                self.requestExecuter[types] = self.poolManager.executePoolTxnRequest
        else:
        ...
        for _, txn in self.poolTxnStore.getAllTxn().items():
            if txn[TXN_TYPE] == NEW_NODE:
                ...
                if nodeName == name:
                    ...
                    and self.config.clientBootStrategy == ClientBootStrategy.PoolTxn:
                self.addNewRole(txn)
    ...
    async def executePoolTxnRequest(self, viewNo, ppTime, req):
        ...
        if op[TXN_TYPE] == NEW_NODE:
            self.addNewNodeAndConnect(op)
                        ...
                        self.config.clientBootStrategy == ClientBootStrategy.PoolTxn:
            self.addNewRole(op)
