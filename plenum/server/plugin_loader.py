            ...
            for mod in mods:
                ...
                for c in classes:
                    if not hasattr(c, self._pluginTypeAttrName):
                        logger.warning("skipping plugin {}[class: {}] because it does not have a '{}' "
                                       "attribute".format(mod, c, self._pluginTypeAttrName))
                    else:
                        typ = c.pluginType
                        if typ not in self._validTypes:
                            logger.warning("skipping plugin '{0}' because it does not have a valid '{1}' "
                            ...
                            if importSuccessful:
                                ...
                                logger.error("** ERROR occurred while loading {} plugin from module {}".
                                             format(c.__name__, mod))
