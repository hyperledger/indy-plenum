...
def isNodeType(baseDir, name):
    filepath = os.path.join(os.path.expanduser(baseDir), name + CLIENT_STACK_SUFFIX)
...
def storeExportedTxns(baseDir, txn):
    storeToFile(baseDir, ExportedTxnFile, txn, None, storeHash=False, isLineNoKey=True)
...
def getAddNewGenNodeCommand(name, verkey, pubkey, stewardkey, nodeip, nodeport, clientip, clientport):
    vnodeip, vnodeport, vclientip, vclientport = getAddGenesisHAs(nodeip, nodeport, clientip, clientport)
    nodeAddr = vnodeip + ":" + vnodeport
    clientAddr = vclientip + ":" + vclientport

    return 'add genesis transaction NEW_NODE with data {"' + name + '": {"verkey": ' + verkey + ',"pubkey": "' + pubkey + '",' \
        '"node_address": "' + nodeAddr + '", "client_address": "' + clientAddr + '"},' \
        '"by": "' + stewardkey + '"}'


def getOldAddNewGenNodeCommand(name, verkey, pubkey, stewardverkey, nodeip, nodeport, clientip, clientport):
    vnodeip, vnodeport, vclientip, vclientport = getAddGenesisHAs(nodeip, nodeport, clientip, clientport)
...
def generateNodeGenesisTxn(baseDir, displayTxn, name, verkey, pubkey, stewardverkey, nodeip, nodeport, clientip, clientport):
    storeNodeInfo(baseDir, name, stewardverkey, nodeip, nodeport, clientip, clientport)
    txn = getOldAddNewGenNodeCommand(name, verkey, pubkey, stewardverkey, nodeip, nodeport, clientip, clientport)
...
def getOldAddNewGenStewardCommand(name, verkey, pubkey):
    return 'add genesis transaction NEW_STEWARD for ' + verkey + ' with data {"alias": "' + name + '", "pubkey": "' + pubkey + '"}'


def generateStewardGenesisTxn(baseDir, displayTxn, name, verkey, pubkey):
    txn = getOldAddNewGenStewardCommand(name, verkey, pubkey)
    storeGenTxns(baseDir, txn)
    printGenTxn(txn, displayTxn)
...
def exportNodeGenTxn(baseDir, displayTxn, name):
    ...
    nodePubKey = getPubKeyFromName(baseDir, name)
    stewardKey = nodeInfo.get('steward')
    nodeAddr = nodeInfo.get('nodeAddr')
    clientAddr = nodeInfo.get('clientAddr')

    txn = 'add genesis transaction NEW_NODE with data {"' + name + '": {"verkey": "' + nodeVerKey + '", "pubkey": "' + nodePubKey + \
