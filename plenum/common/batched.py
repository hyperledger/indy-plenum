...
class Batched(MessageProcessor):
    ...
    def __init__(self, config=None, metrics=MetricsCollector()):
        """
            ...
            if msgs:
                if self._should_batch(msgs):
                    ...
                    if batches:
                        for batch, size in batches:
                            logger.trace("{} sending payload to {}: {}".format(
                                self, dest, batch))
                            self.metrics.add_event(metrics_names.TRANSPORT_BATCH_SIZE, size)
                    ...
                    while msgs:
                        ...
                        self.metrics.add_event(metrics_names.TRANSPORT_BATCH_SIZE, 1)
                        # Setting timeout to never expire
