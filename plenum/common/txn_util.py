...
from plenum.common.constants import TXN_TIME, TXN_TYPE, TARGET_NYM, ROLE, \
    ...
    TXN_PAYLOAD_METADATA_PAYLOAD_DIGEST, TXN_PAYLOAD_METADATA_TAA
...
def append_payload_metadata(
        txn, frm=None, req_id=None, digest=None, payload_digest=None, taa=None):
    if frm is not None:
    ...
    if taa is not None:
        txn[TXN_PAYLOAD][TXN_PAYLOAD_METADATA][TXN_PAYLOAD_METADATA_TAA] = taa
...
def reqToTxn(req):
    ...
    if isinstance(req, dict):
        kwargs = dict(
            ...
            txnAuthrAgrmtMeta=req.get(f.TAA_META.nm, None)
        )
...
def do_req_to_txn(req_data, req_op):
    ...
    append_payload_metadata(result,
                            ...
                            taa=req_data.pop(f.TAA_META.nm, None))
