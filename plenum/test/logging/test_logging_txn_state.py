...
from plenum.test.helper import checkSufficientRepliesReceived, ensureRejectsRecvd
...
def testLoggingTxnStateForValidRequest(looper, steward1, stewardWallet, logsearch):
    logsPropagate, _ = logsearch(levels=['INFO'], files=['propagator.py'],
                    funcs=['propagate'], msgs=['propagating.*request.*from client'])

    logsOrdered, _ = logsearch(levels=['INFO'], files=['replica.py'],
                    funcs=['order_3pc_key'], msgs=['ordered batch request'])

    logsCommited, _ = logsearch(levels=['INFO'], files=['node.py'],
...
def testLoggingTxnStateForInvalidRequest(looper, txnPoolNodeSet,
        clientAndWallet1, client1Connected, logsearch):
    ...
    logsReject, _ = logsearch(levels=['WARNING'], files=['replica.py'],
    ...
    ensureRejectsRecvd(looper, txnPoolNodeSet, client,
...
def testLoggingTxnStateWhenCommitFails(looper, txnPoolNodeSet, steward1, stewardWallet, logsearch):
    logsPropagate, _ = logsearch(levels=['INFO'], files=['propagator.py'],
                    funcs=['propagate'], msgs=['propagating.*request.*from client'])
    ...
    logsCommitFail, _ = logsearch(levels=['WARNING'], files=['node.py'],
    ...
    for node in txnPoolNodeSet:
        ...
        node.executeBatch = functools.partial(executeBatchPatched,
                node.executeBatch)
