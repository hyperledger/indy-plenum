...
def test_send_invalid_txn_author_agreement_fails(
        looper, set_txn_author_agreement_aml, txnPoolNodeSet, vdr_pool_handle, vdr_wallet_trustee, random_taa
):
    req = looper.loop.run_until_complete(
        build_txn_author_agreement_request(vdr_wallet_trustee[1], *random_taa)
    )
    req = json.loads(req)
    req[OPERATION]['text'] = 42
    rep = vdr_sign_and_send_prepared_request(looper, vdr_wallet_trustee, vdr_pool_handle, json.dumps(req))
    with pytest.raises(RequestNackedException):
        vdr_get_and_check_replies(looper, [rep])


def test_create_txn_author_agreement_succeeds(looper, set_txn_author_agreement_aml, vdr_pool_handle, vdr_wallet_trustee):
    # Write random TAA
    version, text, ratified = randomString(16), randomString(1024), get_utc_epoch() - 600
    sdk_send_txn_author_agreement(looper, vdr_pool_handle, vdr_wallet_trustee,
                                  version=version,
                                  text=text,
                                  ratified=ratified)

    # Make sure TAA successfully written as latest TAA
    rep = sdk_get_txn_author_agreement(looper, vdr_pool_handle, vdr_wallet_trustee)[1]
    ...
    rep = sdk_get_txn_author_agreement(looper, vdr_pool_handle, vdr_wallet_trustee, version=version)[1]
    assert rep[OP_FIELD_NAME] == REPLY
...
def test_txn_author_agreement_update_ratification_fails(looper, set_txn_author_agreement_aml,
                                                        vdr_pool_handle, vdr_wallet_trustee, ratified_offset):
    # Write random TAA
    version, text, ratified = randomString(16), randomString(1024), get_utc_epoch() - 600
    sdk_send_txn_author_agreement(looper, vdr_pool_handle, vdr_wallet_trustee,
                                  version=version,
...
def test_txn_author_agreement_retire_non_latest(looper, set_txn_author_agreement_aml,
    ...
    rep = sdk_get_txn_author_agreement(looper, vdr_pool_handle, vdr_wallet_trustee, version=version_1)[1]
    assert rep[OP_FIELD_NAME] == REPLY
    taa = rep['result'][DATA]
    ...
    rep = sdk_get_txn_author_agreement(looper, vdr_pool_handle, vdr_wallet_trustee, version=version_2)[1]
    assert rep[OP_FIELD_NAME] == REPLY
    taa = rep['result'][DATA]
...
def test_send_valid_txn_author_agreement_without_enough_privileges_fails(
        ...
        vdr_pool_handle, vdr_wallet_steward, random_taa
):
    with pytest.raises(RequestRejectedException):
        text, version = random_taa
        sdk_send_txn_author_agreement(looper, vdr_pool_handle, vdr_wallet_steward, version, text)
