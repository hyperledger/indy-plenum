...
def test_backup_primary_restores_pp_seq_no_if_view_is_same(
    ...
    assert backup_primary_node.viewNo == 2
    ...
    restarted_node = start_stopped_node(backup_primary_node, looper,
                                             tconf, tdir, allPluginsPath)
    ...
    assert restarted_node.replicas[backup_inst_id].lastPrePrepareSeqNo == 5
    assert restarted_node.replicas[backup_inst_id].last_ordered_3pc == (2, 5)
