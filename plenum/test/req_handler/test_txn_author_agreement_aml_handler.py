...
@pytest.fixture(scope="function")
def aml_request(tconf, txn_author_agreement_handler, domain_state):
...
def test_static_validation(txn_author_agreement_handler, aml_request):
    txn_author_agreement_handler.static_validation(aml_request)


def test_static_validation_with_empty_aml(txn_author_agreement_handler, aml_request):
...
def test_dynamic_validation(txn_author_agreement_handler, aml_request):
    txn_author_agreement_handler.dynamic_validation(aml_request)


def test_dynamic_validation_with_not_unique_aml(txn_author_agreement_handler, aml_request):
    version = aml_request.operation[AML_VERSION]
    txn_author_agreement_handler.state.set(StaticTAAHelper.state_path_taa_aml_version(version), "{}")
    with pytest.raises(InvalidClientRequest,
                       match="Version of TAA AML must be unique and it cannot be modified"):
        txn_author_agreement_handler.dynamic_validation(aml_request)


def test_dynamic_validation_from_steward(txn_author_agreement_handler, domain_state, aml_request):
