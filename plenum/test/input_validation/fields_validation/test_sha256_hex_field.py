...
def test_sha256_hex_field():
    ...
    assert validator.validate(hex_h512) == 'not a valid hash (needs to be in hex too)'
    hmd5 = hashlib.md5(msg)
    hex_hmd5 = hmd5.hexdigest()
    assert validator.validate(hex_hmd5) == 'not a valid hash (needs to be in hex too)'
    ...
    assert validator.validate(b64_h256) == 'not a valid hash (needs to be in hex too)'
    b58_h256 = base58.b58encode(h256.digest())
    assert validator.validate(b58_h256) == 'not a valid hash (needs to be in hex too)'

    # Confirm that converting to hex by other means also works
