...
def test_non_empty_verkeys():
    for byte_len in range(1, 33):
        ...
        if byte_len == 16:
            ...
            assert as_long == 'b58 decoded value length {} should be one of [32]'.format(byte_len)
            assert as_short == 'b58 decoded value length {} should be one of [16]'.format(byte_len)
