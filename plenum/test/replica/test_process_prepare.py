...
@pytest.mark.pre_prepare_frm(PRIMARY_NAME)
@pytest.mark.prepare_frm(NON_PRIMARY_NAME)
def test_process_valid_prepare(r, pre_prepare, prepare):
    r.processPrePrepare(pre_prepare)
    r.processPrepare(prepare)
    assert r.prepares.hasPrepareFrom(prepare, NON_PRIMARY_NAME)


@pytest.mark.prepare_frm(PRIMARY_NAME)
