...
def test_create_3pc_batch_with_empty_requests(replica):
    pp = replica._ordering_service.l_create_3pc_batch(0)
    assert pp is not None
    assert pp.reqIdr == tuple()


def test_create_3pc_batch(replica_with_requests, fake_requests, txn_roots, state_roots):
    replica_with_requests._ordering_service.l_consume_req_queue_for_pre_prepare = \
    ...
    for (ledger_id, pp_seq_no, has_bls) in [
        ...
        pre_prepare_msg = replica_with_requests._ordering_service.l_create_3pc_batch(ledger_id)
