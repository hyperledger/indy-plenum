...
def test_promotion_leads_to_primary_inconsistency(looper,
    ...
    steward_3_seed = 'Steward3' + '0' * 24
    steward_3_did, _ = looper.loop.run_until_complete(
        create_and_store_my_did(sdk_wallet_handle,
                                json.dumps({'seed': steward_3_seed}))
    )
    demote_node(looper, (sdk_wallet_handle, steward_3_did), sdk_pool_handle, node_3)
    ...
    sdk_send_random_and_check(looper, txnPoolNodeSet, sdk_pool_handle, sdk_wallet_steward, 2)
    ...
    promote_node(looper, (sdk_wallet_handle, steward_3_did), sdk_pool_handle, node_3)
