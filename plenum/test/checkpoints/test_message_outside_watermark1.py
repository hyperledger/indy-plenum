...
def test_primary_recvs_3phase_message_outside_watermarks(perf_chk_patched, chkFreqPatched, looper, txnPoolNodeSet,
    ...
    orderedCount = pr._ordering_service.spylog.count(pr._ordering_service.l_order_3pc_key)
    ...
    def chk():
        assert orderedCount + batch_count == pr._ordering_service.spylog.count(pr._ordering_service.l_order_3pc_key)
