...
def test_request_older_than_stable_checkpoint_removed(chkFreqPatched, looper, txnPoolNodeSet, vdr_pool_handle,
                                                      vdr_wallet_steward, reqs_for_checkpoint):
    ...
    vdr_send_random_and_check(looper, txnPoolNodeSet, vdr_pool_handle, vdr_wallet_steward, 2 * max_batch_size)
    ...
    sdk_wallet_user = vdr_add_new_nym(looper, vdr_pool_handle, vdr_wallet_steward)
    looper.run(eventually(check_for_nodes, txnPoolNodeSet, check_stable_checkpoint, 0, retryWait=1, timeout=timeout))
    ...
    with pytest.raises(RequestRejectedException):
        vdr_add_new_nym(looper, vdr_pool_handle, sdk_wallet_user)
    looper.run(eventually(check_for_nodes, txnPoolNodeSet, check_stable_checkpoint, 0, retryWait=1, timeout=timeout))
    checkRequestCounts(txnPoolNodeSet, 2 * max_batch_size + 2, 4)

    # Send more requests to cause checkpoint stabilization
    vdr_send_random_and_check(looper, txnPoolNodeSet, vdr_pool_handle, vdr_wallet_steward, max_batch_size)
    ...
    vdr_send_random_and_check(looper, txnPoolNodeSet, vdr_pool_handle, vdr_wallet_steward, reqs_for_checkpoint + 1)
