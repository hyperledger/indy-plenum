...
def test_request_older_than_stable_checkpoint_removed(chkFreqPatched, looper, txnPoolNodeSet, sdk_pool_handle,
    ...
    reqs = send_random_and_check(looper, txnPoolNodeSet, sdk_pool_handle, sdk_wallet_client,
    ...
    send_random_and_check(looper, txnPoolNodeSet, sdk_pool_handle, sdk_wallet_client, max_batch_size)
    ...
    send_random_and_check(looper, txnPoolNodeSet, sdk_pool_handle, sdk_wallet_client, reqs_for_checkpoint + 1)
    total_checkpoints = 2
    looper.run(eventually(chkChkpoints, txnPoolNodeSet, total_checkpoints, stable_checkpoint_id,
                          retryWait=1, timeout=timeout))
