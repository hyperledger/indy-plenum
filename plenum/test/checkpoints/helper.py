...
def check_quorum_on_received_checkpoints(replica, have_quorum: bool):
    assert any(replica._checkpointer._have_quorum_on_received_checkpoint(cp)
               for cp in replica._checkpointer._received_checkpoints) == have_quorum


def check_num_unstabilized_checkpoints(replica, num):
