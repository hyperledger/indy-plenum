...
def test_view_change_not_happen_if_ic_is_discarded(looper, txnPoolNodeSet,
    ...
    def check_old_ic_discarded():
        assert all(not n.view_changer.instance_change_provider.has_inst_chng_from(view_no + 1, panic_node.name)
    ...
    def check_ic():
        for node in txnPoolNodeSet:
            assert all(node.view_changer.instance_change_provider.has_inst_chng_from(view_no + 1, n.name)
                       for n in nodes_to_restart)
