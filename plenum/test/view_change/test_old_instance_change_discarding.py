...
def test_old_instance_change_discarding(txnPoolNodeSet,
    ...
    def chk_ic_discard():
        for n in txnPoolNodeSet:
            assert not n.view_changer.instance_change_provider.has_view(view_no + 1)
            for frm in first_nodes:
                assert not n.view_changer.instance_change_provider.has_inst_chng_from(view_no + 1, frm.name)
