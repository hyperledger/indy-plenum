...
def testViewChangesIfMasterPrimaryDisconnected(txnPoolNodeSet,
    ...
    old_pr_node = disconnect_master_primary(txnPoolNodeSet)

    looper.removeProdable(old_pr_node)
    remainingNodes = set(nodes) - {old_pr_node}
    # Sometimes it takes time for nodes to detect disconnection, especially
    # when the whole test package is run.
    ...
    # changed and new primary has been elected and the pool is functional
