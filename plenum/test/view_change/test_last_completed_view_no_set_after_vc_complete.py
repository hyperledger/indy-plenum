...
def test_set_last_completed_view_no_after_vc_complete(txnPoolNodeSet, looper):
    ...
    assert all([old_view == n.master_replica._consensus_data.last_completed_view_no for n in txnPoolNodeSet])
    ...
    assert all([new_view == n.master_replica._consensus_data.last_completed_view_no for n in txnPoolNodeSet])
