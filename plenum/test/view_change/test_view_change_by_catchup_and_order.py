...
def test_view_change_by_order_stashed_on_3_nodes_and_catchup_and_ordered_on_1_node(txnPoolNodeSet, looper,
                                                                                   sdk_pool_handle, sdk_wallet_steward):
    '''
    - COMMITS are delayed on all nodes
    - All nodes starts a view change with a prepared certificate (for delayed message)
    - Some COMMITS come during view change for 3 nodes
    - These 3 nodes order some txns (but not till prepared certificate yet)
    - The lagging (4th) node receives some missing txns as part of catch-up (during view change)
    - All nodes receive COMMITs for the remaining txns for last_prepared certificate
    - All nodes finish view change
    '''
    slow_node = txnPoolNodeSet[-1]
    fast_nodes = txnPoolNodeSet[:-1]
    slow_stasher = slow_node.nodeIbStasher
