...
from plenum.test.helper import sdk_gen_request, sdk_gen_request_plenum
...
def test_audit_ledger_multiple_ledgers_in_one_batch(txnPoolNodeSet):
    ...
    nym_req_plenum = sdk_gen_request_plenum(op, signatures={"sig1": "111"})
    node.write_manager.apply_request(nym_req_plenum, 10000)
    op2 = {TXN_TYPE: TXN_AUTHOR_AGREEMENT_AML,
           AML_VERSION: "version1"}
    pool_config_req = sdk_gen_request_plenum(op2, signatures={"sig1": "111"})
    ...
    nym_req = sdk_gen_request_plenum(op, signatures={"sig1": "111"})
...
def test_multiple_ledgers_in_second_batch_apply_first_time(txnPoolNodeSet):
    ...
    node_req = sdk_gen_request_plenum(op2, signatures={"sig1": "111"})
