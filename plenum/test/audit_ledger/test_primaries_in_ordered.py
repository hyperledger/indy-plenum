...
def test_primaries_in_ordered_from_audit(vdr_test_node):
    pre_prepare = create_pre_prepare_no_bls(state_root=generate_state_root(), pp_seq_no=1)
    replica = vdr_test_node.master_replica
    ...
    vdr_test_node.write_manager.audit_b_handler.post_batch_applied(three_pc_batch)

    assert replica._ordering_service._get_primaries_for_ordered(pre_prepare) == three_pc_batch.primaries


def test_primaries_in_ordered_from_audit_for_tree_txns(vdr_test_node):
    primaries = {}
    replica = vdr_test_node.master_replica
    for i in range(3):
