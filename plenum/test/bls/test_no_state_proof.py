...
from plenum.test.helper import sdk_json_to_plenum_request_object, sdk_signed_random_requests
...
def test_make_proof_bls_disabled(looper, txnPoolNodeSet,
                                 sdk_wallet_client):
    req = json.loads(sdk_signed_random_requests(looper, sdk_wallet_client, 1)[0].body)
...
def test_make_result_bls_disabled(looper, txnPoolNodeSet,
    ...
    for node in txnPoolNodeSet:
        ...
        key = BuyHandler.prepare_buy_key(req['identifier'], req['reqId'])
        _, _, _, proof = req_handler.lookup(key, with_proof=True)
        custom_req = sdk_json_to_plenum_request_object(req)
        result = req_handler.make_result(custom_req, {TXN_TYPE: "buy"}, 2, get_utc_epoch(), proof)
