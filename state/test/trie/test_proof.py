...
def test_verify_proof():
    ...
    p00 = deepcopy(p0)
    assert client_trie.verify_spv_proof(root_hash_0, 'k2'.encode(), p0)
    assert p00 == p0
    ...
    assert client_trie.verify_spv_proof(root_hash_1, 'k1'.encode(), p1)

    p2 = node_trie.produce_spv_proof('x2'.encode())
    p2.append(node_trie.root_node)
    assert client_trie.verify_spv_proof(root_hash_1, 'x2'.encode(), p2)
    ...
    assert client_trie.verify_spv_proof(root_hash_1, 'k3'.encode(), p3)
    assert client_trie.verify_spv_proof(root_hash_1, 'x1'.encode(), p4)
...
def test_verify_proof_random_data():
    ...
    for i in range(0, partitions):
        ...
        assert all([client_trie.verify_spv_proof(root_hashes[i], k, proofs[i][k])
                    for k in keys[i]])
    ...
    for _ in range(400):
        ...
        key = choice(keys[p])
        assert client_trie.verify_spv_proof(root_hashes[p], key, proofs[p][key])
    ...
    for _ in range(400):
        ...
        assert client_trie.verify_spv_proof(root_hash, key, proof)
...
def test_proof_serialize_deserialize():
    ...
    for k in keys:
        ...
        assert client_trie.verify_spv_proof(node_trie.root_hash, k,
                                            prf, serialized=True)
